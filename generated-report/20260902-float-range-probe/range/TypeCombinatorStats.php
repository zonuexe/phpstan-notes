...
final class TypeCombinator
{
	...
	{
		self::$stats["clearCache"] = (self::$stats["clearCache"] ?? 0) + 1;
	...
	{
		self::$stats["addNull"] = (self::$stats["addNull"] ?? 0) + 1;
	...
	{
		self::$stats["remove"] = (self::$stats["remove"] ?? 0) + 1;
	...
	{
		self::$stats["doRemove"] = (self::$stats["doRemove"] ?? 0) + 1;
	...
	{
		self::$stats["removeNull"] = (self::$stats["removeNull"] ?? 0) + 1;
	...
	{
		self::$stats["containsNull"] = (self::$stats["containsNull"] ?? 0) + 1;
	...
	{
		self::$stats["union"] = (self::$stats["union"] ?? 0) + 1;
		if (self::$cacheEnabled ??= TurboExtensionEnabler::isTypeCombinatorCacheEnabled()) {
	...
	{
		self::$stats["doUnion"] = (self::$stats["doUnion"] ?? 0) + 1;
		...
		for ($i = 0; $i < $typesCount; $i++) {
			...
			} elseif ($types[$i]->isFloat()->yes() && !$types[$i] instanceof FloatRangeType) {
			...
			if ($types[$i] instanceof FloatRangeType) {
				$floatRangeTypes[] = $types[$i];
				unset($types[$i]);
		...
		usort(
			$floatRangeTypes,
			static fn (FloatRangeType $a, FloatRangeType $b): int => $a->getMin() <=> $b->getMin() ?: $a->getMax() <=> $b->getMax(),
		);
		// Merge the sorted ranges first so that a NAN constant can still fold `float<-inf, inf>|NAN` into `float` below.
		$mergedFloatRangeTypes = [];
		foreach ($floatRangeTypes as $floatRangeType) {
			$lastIndex = count($mergedFloatRangeTypes) - 1;
			if ($lastIndex >= 0) {
				$merged = $mergedFloatRangeTypes[$lastIndex]->tryUnion($floatRangeType);
				if ($merged instanceof FloatRangeType) {
					$mergedFloatRangeTypes[$lastIndex] = $merged;
					continue;
				}
			}
			$mergedFloatRangeTypes[] = $floatRangeType;
		}
		$types = array_merge($types, $integerRangeTypes, $mergedFloatRangeTypes);
		...
		foreach ($scalarTypes as $scalarTypeItems) {
			foreach ($scalarTypeItems as $scalarType) {
				if ($scalarType instanceof ConstantFloatType && is_nan($scalarType->getValue())) {
					// float ranges may have merged into `float<-inf, inf>` only after the scalar pass above
					foreach ($types as $k => $type) {
						if (!$type instanceof FloatRangeType) {
							continue;
						}
						$folded = $type->tryUnion($scalarType);
						if ($folded !== null) {
							$types[$k] = $folded;
							continue 2;
						}
	...
	{
		self::$stats["isAlreadyNormalized"] = (self::$stats["isAlreadyNormalized"] ?? 0) + 1;
	...
	{
		self::$stats["compareTypesInUnion"] = (self::$stats["compareTypesInUnion"] ?? 0) + 1;
		...
		if ($a instanceof FloatRangeType) {
			$type = $a->tryUnion($b);
			if ($type !== null) {
				return [$type, null];
			}
		}
		if ($b instanceof FloatRangeType) {
			$type = $b->tryUnion($a);
			if ($type !== null) {
				return [null, $type];
			}
		}
		if ($a instanceof FloatRangeType && $b instanceof FloatRangeType) {
			return null;
		}
	...
	{
		self::$stats["getAccessoryCaseStringTypes"] = (self::$stats["getAccessoryCaseStringTypes"] ?? 0) + 1;
		$accessory = [];
	...
	{
		self::$stats["downgradeNonFalsyStringToNonEmpty"] = (self::$stats["downgradeNonFalsyStringToNonEmpty"] ?? 0) + 1;
	...
	{
		self::$stats["removeDecimalIntStringAccessory"] = (self::$stats["removeDecimalIntStringAccessory"] ?? 0) + 1;
	...
	{
		self::$stats["unionWithSubtractedType"] = (self::$stats["unionWithSubtractedType"] ?? 0) + 1;
	...
	{
		self::$stats["intersectWithSubtractedType"] = (self::$stats["intersectWithSubtractedType"] ?? 0) + 1;
	...
	{
		self::$stats["processArrayAccessoryTypes"] = (self::$stats["processArrayAccessoryTypes"] ?? 0) + 1;
	...
	{
		self::$stats["processArrayTypes"] = (self::$stats["processArrayTypes"] ?? 0) + 1;
		if ($arrayTypes === []) {
			return [];
		}
	...
	{
		self::$stats["optimizeConstantArrays"] = (self::$stats["optimizeConstantArrays"] ?? 0) + 1;
	...
	{
		self::$stats["countConstantArrayValueTypes"] = (self::$stats["countConstantArrayValueTypes"] ?? 0) + 1;
	...
	{
		self::$stats["reduceArrays"] = (self::$stats["reduceArrays"] ?? 0) + 1;
		$newArrays = [];
		$arraysToProcess = [];
		$emptyArray = null;
		foreach ($constantArrays as $constantArray) {
	...
	{
		self::$stats["intersectFiniteUnions"] = (self::$stats["intersectFiniteUnions"] ?? 0) + 1;
	...
	{
		self::$stats["finiteUnionMembers"] = (self::$stats["finiteUnionMembers"] ?? 0) + 1;
	...
	{
		self::$stats["intersect"] = (self::$stats["intersect"] ?? 0) + 1;
		if (self::$cacheEnabled ??= TurboExtensionEnabler::isTypeCombinatorCacheEnabled()) {
	...
	{
		self::$stats["doIntersect"] = (self::$stats["doIntersect"] ?? 0) + 1;
		$typesCount = count($types);
		if ($typesCount === 0) {
			return new NeverType();
		}
	...
	{
		self::$stats["intersectDefiniteConstantArrays"] = (self::$stats["intersectDefiniteConstantArrays"] ?? 0) + 1;
	...
	{
		self::$stats["mergeIntersectionsForUnion"] = (self::$stats["mergeIntersectionsForUnion"] ?? 0) + 1;
		$aTypes = $a->getTypes();
		$bTypes = $b->getTypes();
	...
	{
		self::$stats["removeFalsey"] = (self::$stats["removeFalsey"] ?? 0) + 1;
		return self::remove($type, StaticTypeFactory::falsey());
	}

	public static function removeTruthy(Type $type): Type
	{
		self::$stats["removeTruthy"] = (self::$stats["removeTruthy"] ?? 0) + 1;
